...
def get_site_prev_output(cursor: sqlite3.Cursor, site_id: str, timestamp) -> List[float]:
    prev_ght_query = '''
        SELECT
            site_id,
            AVG(ght) AS avg_ght, 
    ...
    result = cursor.execute(prev_ght_query, (timestamp, timestamp, site_id)).fetchone()
